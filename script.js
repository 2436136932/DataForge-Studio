...
// ==========================================================================
// 🚀 一键加载预设演示数据集 (Quick Demo Datasets)
// ==========================================================================
const samplePresets = {
  ecommerce: () => {
    const data = [
      { '订单编号': 'ORD-2026-0801', '客户姓名': '张晓明', '商品类目': '数码科技', '商品名称': '4K OLED 便携显示器', '单价(元)': 1899, '购买数量': 2, '实付金额(元)': 3798, '支付方式': '微信支付', '配送城市': '北京', '订单状态': '已完成' },
      { '订单编号': 'ORD-2026-0802', '客户姓名': '李思雨', '商品类目': '办公设备', '商品名称': '人体工学机械键盘', '单价(元)': 499, '购买数量': 1, '实付金额(元)': 499, '支付方式': '支付宝', '配送城市': '上海', '订单状态': '已发货' },
      { '订单编号': 'ORD-2026-0803', '客户姓名': '王建国', '商品类目': '数码科技', '商品名称': '无线降噪头戴耳机', '单价(元)': 1299, '购买数量': 1, '实付金额(元)': 1299, '支付方式': '云闪付', '配送城市': '深圳', '订单状态': '已完成' },
      { '订单编号': 'ORD-2026-0804', '客户姓名': '陈雅琴', '商品类目': '居家生活', '商品名称': '智能恒温咖啡加热垫', '单价(元)': 159, '购买数量': 3, '实付金额(元)': 477, '支付方式': '微信支付', '配送城市': '杭州', '订单状态': '待付款' },
      { '订单编号': 'ORD-2026-0805', '客户姓名': '刘伟杰', '商品类目': '办公设备', '商品名称': '电动升降办公桌 Pro', '单价(元)': 2599, '购买数量': 1, '实付金额(元)': 2599, '支付方式': '信用卡', '配送城市': '成都', '订单状态': '已完成' }
    ];
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, '销售报表');
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    return new File([wbout], '2026年度电商多维销售报表.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  },

  medical: () => {
    const lines = [
      JSON.stringify({ '题号': 1, '试题类型': '单选题', '试卷科目': '临床医学综合', '题目干': '下列哪项是急性心肌梗死最早出现的标志物？', '选项A': '肌红蛋白 (Myoglobin)', '选项B': '肌钙蛋白 I (cTnI)', '选项C': '肌钙蛋白 T (cTnT)', '选项D': '肌酸激酶同工酶 (CK-MB)', '标准答案': 'A', '考点归属': '心血管内科学·心肌损伤标志物', '名师解析': '肌红蛋白在急性心肌梗死发生后 1~2 小时即开始升高，是出现最早的标志物；而 cTnI 和 cTnT 升高稍晚但特异性最高。' }),
      JSON.stringify({ '题号': 2, '试题类型': '单选题', '试卷科目': '病理生理学', '题目干': '引起支气管哮喘发作的主要炎性细胞是？', '选项A': '嗜酸性粒细胞与肥大细胞', '选项B': '中性粒细胞', '选项C': '单核细胞', '选项D': '浆细胞', '标准答案': 'A', '考点归属': '呼吸系统疾病·哮喘病理机制', '名师解析': '支气管哮喘是由嗜酸性粒细胞、肥大细胞和 T 淋巴细胞等多种炎性细胞参与的气道慢性炎症。' }),
      JSON.stringify({ '题号': 3, '试题类型': '单选题', '试卷科目': '药理学基础', '题目干': '阿司匹林抗血栓形成的机制是？', '选项A': '抑制环氧酶，减少 TXA2 生成', '选项B': '促进前列环素 PGI2 合成', '选项C': '直接对抗凝血酶作用', '选项D': '抑制维生素 K 还原', '标准答案': 'A', '考点归属': '解热镇痛抗炎药·血小板抑制', '名师解析': '小剂量阿司匹林通过不可逆抑制血小板 COX-1 酶，减少血栓素 A2 (TXA2) 的生成，发挥抗血小板聚集与抗血栓形成的作用。' }),
      JSON.stringify({ '题号': 4, '试题类型': '单选题', '试卷科目': '内科学', '题目干': '诊断糖尿病的主要依据是？', '选项A': '空腹血糖与 OGTT 糖耐量试验', '选项B': '尿糖定性阳性', '选项C': '糖化血红蛋白偏低', '选项D': '多饮、多食、多尿症状', '标准答案': 'A', '考点归属': '内分泌代谢·糖尿病诊断标准', '名师解析': '糖尿病的诊断以静脉血浆葡萄糖水平（空腹血糖 ≥ 7.0mmol/L 或 OGTT 2h 血糖 ≥ 11.1mmol/L）为主要依据，尿糖仅作为初筛参考。' })
    ];
    return new File([lines.join('\n')], '2026年医学资格考试精选题库.jsonl', { type: 'application/x-jsonlines' });
  },

  strategy: () => {
    const text = `# 2026-2028 年度企业数字化战略转型规划

## 1. 战略愿景与发展背景
随着云原生架构与生成式 AI 技术的深度渗透，企业需加速构建端到端的本地智能工作流与数据孤岛打通引擎。

## 2. 核心架构与产品矩阵
全面升级全栈文档与数据沙箱转换底座，实现跨 Office（Word/PPT/Excel）与现代结构化数据（JSON/JSONL/Markdown）的秒级无缝转换与高保真仿真呈现。

## 3. 商业落地与业务增长目标
第一阶段完成核心办公场景 100% 覆盖，第二阶段打通知识库与大模型微调训练集流水线，实现数据流转效率提升 300%。

## 4. 关键里程碑与交付路线
Q1 季度完成多格式内核与视觉仿真引擎上线；Q2 季度构建本地轻量级多端部署生态；Q3 季度全面进入全行业推广。`;
    return new File([text], '2026企业数字化战略转型规划纲要.md', { type: 'text/markdown' });
  }
};
...
document.querySelectorAll('.demo-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const factory = samplePresets[demoType];
    if (factory) {
      const file = factory();
